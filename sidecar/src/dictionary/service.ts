...
export class DictionaryService {
  ...
  async read(payload: {
    ...
    const filtered = query
      ? entries.filter((entry) =>
          `${entry.type}\t${entry.simplified}\t${entry.traditional}`.toLowerCase().includes(query),
        )
      : entries;
    const sorted = [...filtered].sort((left, right) => {
      if (payload.sort === "s2t")
        return (
          right.simplifiedPriority - left.simplifiedPriority ||
          right.simplified.length - left.simplified.length ||
          left.index - right.index
        );
      if (payload.sort === "t2s")
        return (
          right.traditionalPriority - left.traditionalPriority ||
  ...
  async update(payload: { path: string } & DictionaryChanges) {
    if (!payload.path)
      throw new ConvertZZError("DICTIONARY_PATH", "儲存字典前必須選取可寫入檔案。");
    const raw = (await readFile(payload.path, "utf8")).replace(/^\uFEFF/, "");
    const lines = raw.split(/\r?\n/);
    for (const update of payload.updates ?? []) {
      if (update.index < 0 || update.index >= lines.length)
        throw new ConvertZZError("DICTIONARY_INDEX", "字典項目索引已失效。請重新載入。");
    ...
    const transactionBackup = join(
      dirname(payload.path),
      `.convertzz-dictionary-original-${randomUUID()}.csv`,
    );
    try {
  ...
  async preview(
    payload: { path?: string; text: string; direction: Direction } & DictionaryChanges,
  ) {
    const path = payload.path || this.defaultPath;
    if (!path) throw new ConvertZZError("DICTIONARY_MISSING", "找不到字典路徑。");
    ...
    const fallback = (text: string) =>
      payload.direction === "s2t" ? cn2tw(text) : payload.direction === "t2s" ? tw2cn(text) : text;
...
function serializeEntry(entry: DictionaryEntry): string {
  if (!entry.simplified || !entry.traditional)
    throw new ConvertZZError("DICTIONARY_ENTRY", "簡體與繁體欄位不能留空。");
  return [
    entry.enabled,
    entry.type,
    entry.simplified,
    entry.simplifiedPriority,
    entry.traditional,
    entry.traditionalPriority,
  ].join("\t");
}
