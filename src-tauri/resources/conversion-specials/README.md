...
這層與套件 `segment-dict`、語料回環 `extra-correction` 分開。語料產生的整詞同義詞仍放 extra-correction。`pin` 與 `word=`／`word^=` 列出的 2 字以上詞會在分詞時釘入（固定 extra），轉換繁體時仍走 skip／s2t-multi。台灣完整「xx鄉／xx里」在 `place-names.txt`，同樣釘入分詞、簡轉繁拉回正名，`roundtrip-dict` 寫回 extra 分詞表時也會帶上。不要把規則寫進 `conversion.rs`。
...
- **台灣 xx鄉／xx里**：`place-names.txt` 完整行政區名（三星鄉、莊敬里），不收鎮／市／區／村。
...
`place-names.txt` 一列一個完整名稱，只收「xx鄉」「xx里」（不收鎮／市／區／村或單字）。來源是內政部國土測繪中心代碼服務。更新：

```bash
python3 src-tauri/resources/conversion-specials/fetch-place-names.py
```

載入後會釘入分詞，並為簡體「乡」、詞尾「裡／裏」、「臺／台」、「庄／莊」準備對應詞形；簡轉繁時拉回名單上的正名（例如 莊敬裡→莊敬里、三星乡→三星鄉、水裡鄉→水里鄉、南莊鄉→南庄鄉）。「夢裡里」這類正名本身含「裡」的維持原字。方位詞「這裡／家裡／公里」不在名單裡，不會被改成村里用字。
