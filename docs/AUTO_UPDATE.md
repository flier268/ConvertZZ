...
程式預設只檢查正式版。設定中的「檢查開發／預發佈版本」開啟後，會另外讀取 GitHub 上的 `alpha`、`beta`、`rc` 通道標籤（附註訊息為實際版本，例如 `v2.0.0-beta1`），再與正式版比較，取 SemVer 較新者。排序為 `2.0.0 > 2.0.0-rc.1 > 2.0.0-beta8 > 2.0.0-beta7 > 2.0.0-alpha9`。因此若正式版已經比目前的開發版新，仍會提示下載正式版。
