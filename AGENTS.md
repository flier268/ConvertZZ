...
| `src-tauri/resources/conversion-specials/` | 字形特例與固定整詞（`rules.txt`、`place-names.txt`）：簡轉繁、一簡多繁、一繁多簡、繁轉簡、異體、skip、pin。`pin`／`word=`／完整 xx鄉／xx里 在分詞時釘入、轉換時再套用；roundtrip-dict 寫回 extra 分詞表；不寫入套件詞典 |
...
- 新式引擎只使用 `ws-segment-rs` 的分詞與 `ZhtSynonymOptimizer`，字形只交給 `cjk-convert-rs`。不要新增硬編碼語意取代清單。璇／疱／么、胜肽／里長與一簡多繁等字形特例與固定整詞只寫 `resources/conversion-specials/rules.txt`；台灣完整「xx鄉／xx里」寫 `place-names.txt`（分詞釘詞、轉換套用、roundtrip-dict 參照）。
