...
describe("版本檢查", () => {
  it("比較正式版本與預發佈版本", () => {
    ...
    expect(compareVersions("2.0.0", "2.0.0-beta2")).toBe(1);
    expect(compareVersions("2.0.0-beta2", "2.0.0-beta1")).toBe(1);
    expect(compareVersions("2.0.0-beta1", "2.0.0-alpha9")).toBe(1);
    expect(compareVersions("2.0.0-alpha9", "2.0.0-beta1")).toBe(-1);
    expect(compareVersions("v2.0.0-beta2", "2.0.0-beta1")).toBe(1);
  ...
  it("開啟開發版檢查時會選出較新的預發佈版本", async () => {
          ...
          JSON.stringify([
            {
              ...
              html_url: "https://github.com/flier268/ConvertZZ/releases/tag/v2.0.0-beta1",
              prerelease: true,
              draft: false,
            },
            {
              tag_name: "v2.0.0-beta2",
              html_url: "https://github.com/flier268/ConvertZZ/releases/tag/v2.0.0-beta2",
              prerelease: true,
              draft: false,
            },
            {
    ...
    await expect(
      checkLatestRelease("2.0.0-beta1", fetcher as typeof fetch, { includePreRelease: true }),
    ).resolves.toMatchObject({
      latestVersion: "2.0.0-beta2",
      updateAvailable: true,
      url: "https://github.com/flier268/ConvertZZ/releases/tag/v2.0.0-beta2",
    });
    expect(fetcher).toHaveBeenCalledWith(
