...
describe("版本檢查", () => {
  it("比較正式版本與預發佈版本", () => {
    ...
    expect(compareVersions("2.0.0", "2.0.0-beta7")).toBe(1);
    expect(compareVersions("2.0.0-beta7", "2.0.0-beta6")).toBe(1);
    expect(compareVersions("2.0.0-beta1", "2.0.0-alpha9")).toBe(1);
    expect(compareVersions("2.0.0-alpha9", "2.0.0-beta1")).toBe(-1);
    expect(compareVersions("v2.0.0-beta7", "2.0.0-beta6")).toBe(1);
  ...
  it("辨識 alpha／beta／rc 通道標籤", () => {
    ...
    expect(channelFromReleaseTag("2.0.0-beta7")).toBe("beta");
  ...
  it("開啟開發版檢查時會讀取 alpha／beta／rc 通道標籤", async () => {
    const fetcher = githubFetcher({
      ...
      "/git/tags/beta-tag-sha": { message: "v2.0.0-beta7\n" },
    });
    await expect(
      checkLatestRelease("2.0.0-beta6", fetcher, { includePreRelease: true }),
    ).resolves.toMatchObject({
      latestVersion: "2.0.0-beta7",
      updateAvailable: true,
      url: "https://github.com/flier268/ConvertZZ/releases/tag/v2.0.0-beta7",
    });
