...
describe("浮動球右鍵選單", () => {
  it("包含舊版懸浮球右鍵的轉換與視窗動作", () => {
    expect(collectMenuActionIds(FLOATING_CONTEXT_MENU)).toEqual(expect.arrayContaining([
      "a1", "a2", "a3", "a4",
      "b1", "b2",
      "c1", "c2", "c3",
      "za1", "za2", "za3",
  ...
  it("文件、剪貼簿與音訊項目會開啟主視窗對應頁面", () => {
    ...
    expect(resolveShellAction("report")).toEqual({ type: "open-url", url: "https://github.com/flier268/ConvertZZ/issues" });
