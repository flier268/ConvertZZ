...
describe("版本檢查", () => {
  it("比較正式版本與預發佈版本", () => {
    ...
    expect(compareVersions("2.0.0", "2.0.0-beta3")).toBe(1);
    expect(compareVersions("2.0.0-beta3", "2.0.0-beta2")).toBe(1);
    expect(compareVersions("2.0.0-beta1", "2.0.0-alpha9")).toBe(1);
    expect(compareVersions("2.0.0-alpha9", "2.0.0-beta1")).toBe(-1);
    expect(compareVersions("v2.0.0-beta3", "2.0.0-beta2")).toBe(1);
  ...
  it("開啟開發版檢查時會選出較新的預發佈版本", async () => {
    ...
    await expect(
      checkLatestRelease("2.0.0-beta2", fetcher as typeof fetch, { includePreRelease: true }),
    ).resolves.toMatchObject({
      latestVersion: "2.0.0-beta3",
      updateAvailable: true,
      url: "https://github.com/flier268/ConvertZZ/releases/tag/v2.0.0-beta3",
    });
    expect(fetcher).toHaveBeenCalledWith(
